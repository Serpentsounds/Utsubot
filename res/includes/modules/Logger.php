...
Class Logger extends ModuleWithAccounts {
	...
	public function logs(IRCMessage $msg) {
		...
		if (!isset($parameters[0]) || !strlen($parameters[0])) {
			...
			$this->respond($msg, "Usage summary for ". self::bold($msg->getNick()). ": ". implode(", ", $count));
		}
		...
		//	A parameter was given
			...
			if (!$all) {
				...
				$this->respond($msg, $msg->getNick(). " has used '$cmd' ". self::bold(count($logs)). " times.");
			}
			...
			//	Public usage stats
				...
				$this->respond($msg, self::bold($cmd). " has been used ". self::bold(count($logs)). " times.");
