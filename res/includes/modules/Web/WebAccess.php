...
class WebAccess {
	...
	public static function stripHTML($html) {
		...
		$html = preg_replace_callback("/<b>(.*?)<\/b>/i", function ($match) { return IRCUtility::bold($match[1]); }, $html);
		$html = preg_replace_callback("/<i>(.*?)<\/i>/i", function ($match) { return IRCUtility::italic($match[1]); }, $html);
		$html = preg_replace_callback("/<u>(.*?)<\/u>/i", function ($match) { return IRCUtility::underline($match[1]); }, $html);
