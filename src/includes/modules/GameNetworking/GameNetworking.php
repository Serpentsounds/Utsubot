 ...
 * @package Utsubot\GameNetworking
 */
class GameNetworking extends ModuleWithPermission implements IHelp {
    ...
    /**
     * @param IRCBot $irc
     * @throws GameNetworkingException If Users module isn't loaded (required for hybrid nickname/account storage
     *                                 features)
     */
    public function __construct(IRCBot $irc) {
        ...
        //  Help entry
        $help = new HelpEntry("Game Networking", $code);
        $help->addParameterTextPair("NAME TYPE", "Look up NAME's network code for TYPE.");
        $help->addParameterTextPair("NAME", "Look up all of NAME's network codes on record.");
        $help->addParameterTextPair("TYPE", "Look up the user's network code for TYPE.");
        $help->addParameterTextPair("", "Look up all of the user's network codes on record.");
        ...
        $help->addNotes("Valid code TYPE parameters: ".implode(", ", $codeTitles).".");
        $this->addHelp($help);
    }
    ...
    /**
     * If a user is logged in to an account but has codes stored in nickname mode, this command will transfer the
     * entries over to the account
     *
     * @param IRCMessage $msg
     * @throws GameNetworkingException User object retrieval fail, no registered nickname, not logged in, no codes to
    ...
    public function migrateCode(IRCMessage $msg) {
        ...
        $this->respond($msg,
                       sprintf("%d codes were migrated to %s's account.",
                               bold($rowCount), bold($msg->getNick()))
        );
    }
    ...
    public function addCode(IRCMessage $msg) {
        ...
        $this->respond($msg,
                       sprintf("%s has been added as a(n) %s code for %s.",
                               bold($input[ 'code' ]), bold($this->validCodes[ $input[ 'codeID' ] ]->getTitle()), bold($msg->getNick()))
        );
    }
