...
class Calculator {
    ...
    private static $validFunctions = array(
    ...
    public function setExpression($expression) {
        //  Replace math constants and distributed multiplication
        $this->expression = preg_replace_callback("/\b(pi|euler|e)\b/",
            function ($match) { return constant("M_".strtoupper($match[1])); },
            preg_replace("/([0-9\. \)]+)(?=[a-z\(])|([a-z \)]+)(?=\()/", '$1$2*',
                preg_replace("/\s/", "", strtolower($expression))
        ));
    }
	...
	public static function evaluate($expression) {
        ...
        foreach (self::$validFunctions as $function) {
            //  Find position of function call
            $pos = strpos($expression, $function."(");
            if ($pos !== false) {
                //  Start evaluating after opening parentheses
                $startPos = $pos + strlen($function) + 1;
                ...
                //  Invalid characters in expression (nested functions), evaluate recursively
                ...
                $expression = substr($expression, 0, $pos). $evaluate. substr($expression, $startPos + $length + 1);
            }
