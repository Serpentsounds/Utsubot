...
class Web extends ModuleWithPermission {
	...
	public function weather(IRCMessage $msg) {
		$this->_require("Utsubot\\Web\\Weather");
	...
	public function google(IRCMessage $msg) {
		$this->_require("Utsubot\\Web\\Google");
	...
	public function rdns(IRCMessage $msg) {
		...
		$output = array(
			sprintf("%s: %s [%s]", bold("Country"), $results['country'], $results['countryCode']),
			sprintf("%s: %s [%s]", bold("Region"), $results['regionName'], $results['region']),
			sprintf("%s: %s [%s]", bold("City"), $results['city'], $results['zip']),
			sprintf("%s: %s°%s, %s°%s", bold("Location"), round(abs($results['lat']), 2), (($results['lat'] < 0) ? "S" : "N"), round(abs($results['lon']), 2), (($results['lon'] < 0) ? "W" : "E")),
			sprintf("%s: %s", bold("Time Zone"), str_replace("_", " ", $results['timezone'])),
			sprintf("%s: %s", bold("ISP"), $results['isp'])
		);
		...
		for ($i = 0; $i < $count; $i++) {
			...
			if ($items[$i]['id']['kind'] == "youtube#video") {
				$output[] = sprintf(
					...
					$items[$i]['id']['videoId'], self::$separator, bold($items[$i]['snippet']['title']), self::$separator, $items[$i]['snippet']['description']
				);
			}
