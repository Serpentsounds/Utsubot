...
class URLParser extends WebModule {
    ...
    public function __construct(IRCBot $IRCBot) {
        ...
        $this->URLRegexes = [
            new URLParserRegex("youtube.com", "/^watch\?.*?v=([^#&?\/]+)/i", [ $this, "youtube" ], "youtube"),
            new URLParserRegex("youtu.be", "/^([^#&?\/]+)/", [ $this, "youtube" ], "youtube"),
            new URLParserRegex("wikipedia.org", "/^wiki\/([^#&?]+)/", [ $this, "wikipedia" ], "wikipedia"),
            new URLParserRegex("soundcloud.com", "/^([^\/]+\/.+)/", [ $this, "soundcloud" ], "soundcloud"),
        ];
     ...
     * @param IRCMessage $msg
     * @return string
     * @throws URLParserException If content type is unable to be determined, or if the URL string can't be parsed
     *                            properly
     */
    ...
    public function youtube($match, $channel) {
        ...
        $views    = italic(number_format((float)$info[ 'statistics' ][ 'viewCount' ]));
        $likes    = italic(number_format((float)$info[ 'statistics' ][ 'likeCount' ]));
        $dislikes = italic(number_format((float)$info[ 'statistics' ][ 'dislikeCount' ]));
    ...
    public function soundcloud($match, $channel) {
        ...
        $plays     = italic(number_format((float)$json[ 'playback_count' ]));
     ...
     * @param string $url
     * @param string $channel        Output channel name (for caching)
     * @param bool   $skipSimilar    True to abort if the URL and title are deemed "too similar" to provide any new
     *                               information
     * @param float  $similarPercent The minimum percentage of words in the title that must also be found in the URL
     *                               for it to be considered "too similar" (default 70%)
     *
