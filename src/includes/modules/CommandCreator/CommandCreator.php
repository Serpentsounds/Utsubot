...
class CommandCreator extends ModuleWithPermission implements IHelp {
    ...
    public function addCommand(IRCMessage $msg) {
        ...
        //  Type cast to catch null values
        $type = CommandType::fromName((string)array_shift($parameters));
    ...
    protected function triggerCommand(int $commandID, IRCMessage $msg) {
        ...
        $subParameters = function ($match) use ($commandID, $listItems, $msg) {
            ...
            //  File in other dynamic parameters
            switch ($match[ 1 ]) {
                //  Calling user's nickname
                case "n":
                    return $msg->getNick();
                    break;
                ...
                //  A random user on that channel
                case "u":
                    if (!$msg->inChannel())
                        throw new CommandCreatorException("This command must be used in a channel.");

                    /** @var \Utsubot\Channel $channel */
                    $channel = $this->IRCBot->getChannels()->findFirst($msg->getResponseTarget());
                    $users = $channel->getUsers();

                    /** @var \Utsubot\User $randomUser */
                    $randomUser = $users[array_rand($users)];
                    return $randomUser->getNick();
                    break;
            }

            //  A random number
            if (preg_match("/^d+$/", $match[ 1 ])) {
                $maxDigits = strlen($match[ 1 ]);
                $maxValue = (int)str_repeat("9", $maxDigits);
                return mt_rand(1, $maxValue);
            }
        ...
        $output = preg_replace_callback("/%(\d+|[nctu]|d+)/", $subParameters, $commandInfo[ 'format' ]);
    ...
    public function updateCustomTriggerCache() {
        ...
        foreach ($triggers as $row)
            $this->customTriggers[ $row[ 'custom_command_id' ] ][] = $row[ 'trigger' ];
    }
