...
use Utsubot\Converter\ConverterException;
...
class Pokemon extends PokemonBase implements Manageable {
		...
		if (isset($evoArray[$evo])) {
			...
			foreach ($evoArray[$evo] as $method => $details) {
				...
				//	Join all sets of details
					...
					$conditions[$method] = implode(italic(" OR "), $conditions[$method]);
			}

			//	Join conditions for each method
			$conditions = implode(italic(" OR "), $conditions);
	...
	public function getHeight($unit = "meter") {
		if (in_array($unit, array("meter", "metre", "m")))
            ...
            return (new Converter("distance", "meter", $unit))->convert($this->height);        
	}
	...
	public function getWeight($unit = "kilogram") {
		if (in_array($unit, array("kilogram", "kg")))
            ...
            return (new Converter("mass", "kilogram", $unit))->convert($this->weight);
	}
	...
	public function setHeight($height, $unit = "meter") {
		...
		if (in_array($unit, array("meter", "metre", "m")))
			...
			$this->height = (new Converter("distance", "meter", $unit))->convert($height);;
	...
	public function setWeight($weight, $unit = "kilogram") {
		...
		if (in_array($unit, array("kilogram", "kg")))
			...
			$this->weight = (new Converter("mass", "kilogram", $unit))->convert($weight);
