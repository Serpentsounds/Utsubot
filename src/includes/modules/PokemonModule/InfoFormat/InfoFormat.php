...
use function Utsubot\{color, bold};
...
abstract class InfoFormat {
	...
	public function __construct($object) {
		$qualifiedName = "Utsubot\\Pokemon\\". static::$class;
		if (!($object instanceof $qualifiedName))
			throw new InfoFormatException("Object is not an instance of '$qualifiedName'.");
	...
	public function parseFormat($format = null)	{
		...
		$format = preg_replace_callback('/\^([^\^]*)\^/',
			function ($match) use ($headerColors) {
				return ($headerColors[0] === false) ? $match[1] : color($match[1], $headerColors[0], $headerColors[1]);
			},
			$format);
