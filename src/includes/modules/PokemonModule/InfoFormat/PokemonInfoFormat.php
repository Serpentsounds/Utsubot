...
EOF;
	...
	protected function getField($field) {
		switch ($field) {
			...
			break;
			case "roumaji":
				return ucfirst(romanizeKana($this->object->getName("japanese")));
			break;
			...
			case "height":
				...
				switch ($this->units) {
					...
					case "both":
						$value = implode("/", array(
							bold(round($this->object->getHeight("ft"), 2). "ft"),
							bold(round($this->object->getHeight("m"), 2). "m")
						));
					break;
			...
			case "weight":
				...
				switch ($this->units) {
					...
					case "both":
						$value = implode("/", array(
							bold(round($this->object->getWeight("lb"), 2). "lb"),
							bold(round($this->object->getWeight("kg"), 2). "kg")
						));
					break;
			...
			case "preevolution":
				...
				foreach ($names as $key => $name)
					$names[$key] = bold($name). "/". $methods[$key];
