...
namespace Utsubot\Pokemon\StatCalculator;

class StatCalculatorException extends \Exception {}


function baseToMax($base, $level = 100, $HP = false) {
	return calculateStat($base, 31, 252, $level, 1.1, $HP);
}

function maxToBase($max, $level = 100, $HP = false) {
	return calculateBase($max, 31, 252, $level, 1.1, $HP);
}

function calculateStat($base, $IV, $EV, $level, $natureModifier = 1.0, $HP = false) {
	validateParameters($base, $IV, $EV, $level, $natureModifier);
...
function calculateBase($stat, $IV, $EV, $level, $natureModifier = 1.0, $HP = false) {
	validateParameters($stat, $IV, $EV, $level, $natureModifier);
...
function calculateIVs($baseStats, $statValues, $EVs, $level, $natureModifiers) {
	$IVRange = array();
	for ($i = 0; $i <= 5; $i++) {
		validateParameters($statValues[$i], null, $EVs[$i], $level, $natureModifiers[$i]);
...
function validateParameters($stat, $IV, $EV, $level, $natureModifier) {
